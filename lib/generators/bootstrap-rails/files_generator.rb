module BootstrapRails
  module Generators
    class FilesGenerator < Rails::Generators::Base
      ...
      desc "Creates files needed for working with bootstrap-rails. It includes images, partials, javascripts and CSS files."
      
      def generate_files
        source_folder = File.join(File.dirname(File.absolute_path(__FILE__)), "..", "..", "..", "vendor", "assets", "images")
        dest_folder = File.join("vendor", "assets", "images")
        ...
        source_folder = File.join(File.dirname(File.absolute_path(__FILE__)), "..", "..", "..", "vendor", "assets", "javascripts")
        dest_folder = File.join("vendor", "assets", "javascripts")
        Dir.entries(source_folder).each do |file|          
          copy_file("#{source_folder}/#{file}", "#{dest_folder}/#{file}") unless File.directory? "#{source_folder}/#{file}"
        end

        source_folder = File.join(File.dirname(File.absolute_path(__FILE__)), "..", "..", "..", "vendor", "assets", "stylesheets")
        dest_folder = File.join("vendor", "assets", "stylesheets")
        Dir.entries(source_folder).each do |file|          
          copy_file("#{source_folder}/#{file}", "#{dest_folder}/#{file}") unless File.directory? "#{source_folder}/#{file}"
        end
